...
import { dbService } from '../services/storage'; // Importando para Backup
...
const SETTINGS_TABS = [
  ...
  { id: 'backup', label: '💾 Backup & Dados', reqPerms: ['settings:manage_backup'] }, // NOVO
];
...
export const Settings: React.FC<SettingsProps> = ({ settings, setSettings, showToast, hasPermission }) => {
  ...
  return (
    <div className="space-y-6">
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-fadeIn">
               ...
               <div className="bg-white rounded-xl shadow-lg border border-gray-100 p-6 flex flex-col md:col-span-2">
                   ...
                   <div className="bg-indigo-50 p-4 rounded-lg border border-indigo-100">
                       ...
                       />
                       <p className="text-xs text-indigo-600 mt-2">Usuários com acesso a estas filiais verão o gráfico "Banco de Horas" no Dashboard.</p>
                   </div>
               </div>
               )}
               ...
               <div className="bg-white rounded-xl shadow-lg border border-gray-100 p-6 flex flex-col md:col-span-2">
                   ...
                   <div className="flex gap-2 mb-6">
                       <div className="relative flex-1">
                           <input type="text" disabled={!hasPermission('settings:edit_branches')} value={newCompany} onChange={e => { setNewCompany(e.target.value); validate('newCompany', e.target.value); }} onKeyDown={e => e.key === 'Enter' && addCompany()} placeholder="Nova Empresa..." className={`w-full border rounded-lg p-2.5 text-sm outline-none focus:ring-2 focus:ring-indigo-500 transition-all ${errors.newCompany ? 'border-red-300 ring-2 ring-red-100' : 'border-gray-300'} disabled:bg-gray-100 disabled:text-gray-500`} />
                       </div>
                       <button disabled={!hasPermission('settings:edit_branches')} onClick={addCompany} className="bg-indigo-600 text-white px-4 rounded-lg font-bold hover:bg-indigo-700 shadow-sm transition-transform active:scale-95 h-[42px] disabled:opacity-50 disabled:cursor-not-allowed disabled:bg-gray-400">{Icons.Plus}</button>
                   </div>
                   ...
                   <div className="flex flex-col md:flex-row gap-2 mb-6 items-end">
                       {(settings.companies || []).length > 0 && (
                           <div className="w-full md:w-1/3">
                               ...
                               <select className="w-full border border-gray-300 rounded-lg p-2.5 text-sm outline-none bg-white" value={selectedCompanyForBranch} onChange={e => setSelectedCompanyForBranch(e.target.value)}>
                       ...
                       <button disabled={!hasPermission('settings:edit_branches')} onClick={addBranch} className="bg-indigo-600 text-white px-4 rounded-lg font-bold hover:bg-indigo-700 shadow-sm transition-transform active:scale-95 h-[42px] disabled:opacity-50 disabled:cursor-not-allowed disabled:bg-gray-400" title={!hasPermission('settings:edit_branches') ? "Sem permissão para adicionar" : ""}>{Icons.Plus}</button>
                   </div>
                   <div className="flex flex-wrap gap-2 overflow-y-auto max-h-[300px] p-1">
                       {settings.branches.map(b => {
                           ...
                           <div key={b} className="bg-white text-gray-700 pl-3 pr-2 py-1.5 rounded-full flex items-center gap-2 text-sm border border-gray-200 shadow-sm hover:border-indigo-300 transition-colors group">
                               ...
                               <button disabled={!hasPermission('settings:edit_branches')} onClick={() => removeBranch(b)} className="text-gray-400 hover:text-red-500 bg-gray-50 hover:bg-red-50 rounded-full p-1 transition-colors disabled:opacity-30 disabled:hover:bg-transparent disabled:hover:text-gray-400 disabled:cursor-not-allowed" title={!hasPermission('settings:edit_branches') ? "Sem permissão para remover" : "Remover"}><svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg></button>
                           </div>
                       )})}
                   ...
                   <div className="flex flex-col md:flex-row gap-4 mb-6 items-end bg-gray-50 p-4 rounded-xl border border-gray-200">
                       {(settings.companies || []).length > 0 && (
                           <div className="w-full md:w-1/4">
                               ...
                               <select className="w-full border border-gray-300 rounded-lg p-2 text-sm outline-none bg-white" value={sectorCompanyFilter} onChange={e => { setSectorCompanyFilter(e.target.value); setSectorBranchFilter(''); }}><option value="">Todas</option>{settings.companies?.map(c => <option key={c} value={c}>{c}</option>)}</select>
                           </div>
                       )}
                       <div className="w-full md:w-1/3">
                           <label className="text-xs font-bold text-gray-500 uppercase block mb-1">Selecione a Filial *</label>
                           <select className="w-full border border-gray-300 rounded-lg p-2 text-sm outline-none bg-white" value={sectorBranchFilter} onChange={e => setSectorBranchFilter(e.target.value)}><option value="">Selecione...</option>{branchOptionsForSector.map(b => (<option key={b} value={b}>{b}</option>))}</select>
                       </div>
                       <div className="relative flex-1 w-full">
                           <label className="text-xs font-bold text-gray-500 uppercase block mb-1">Novo Setor</label>
                           <input type="text" disabled={!hasPermission('settings:edit_sectors') || !sectorBranchFilter} value={newSector} onChange={e => { setNewSector(e.target.value); validate('newSector', e.target.value); }} onKeyDown={e => e.key === 'Enter' && addSector()} placeholder={!sectorBranchFilter ? "Selecione uma filial..." : "Nome do Setor..."} className={`w-full border rounded-lg p-2 text-sm outline-none focus:ring-2 focus:ring-indigo-500 transition-all ${errors.newSector ? 'border-red-300 ring-2 ring-red-100' : 'border-gray-300'} disabled:bg-gray-100 disabled:text-gray-500`} />
                       </div>
                       <button disabled={!hasPermission('settings:edit_sectors') || !sectorBranchFilter} onClick={addSector} className="bg-indigo-600 text-white px-4 rounded-lg font-bold hover:bg-indigo-700 shadow-sm transition-transform active:scale-95 h-[38px] disabled:opacity-50 disabled:cursor-not-allowed disabled:bg-gray-400" title={!hasPermission('settings:edit_sectors') ? "Sem permissão" : (!sectorBranchFilter ? "Selecione uma filial" : "Adicionar")}>{Icons.Plus}</button>
                   </div>
                           ...
                           <>
                               {(settings.branchSectors?.[sectorBranchFilter] || []).map(s => (
                                   <div key={s} className="bg-white text-gray-700 pl-3 pr-2 py-1.5 rounded-full flex items-center gap-2 text-sm border border-gray-200 shadow-sm hover:border-indigo-300 transition-colors group">
                                       ...
                                       <button disabled={!hasPermission('settings:edit_sectors')} onClick={() => removeSector(sectorBranchFilter, s)} className="text-gray-400 hover:text-red-500 bg-gray-50 hover:bg-red-50 rounded-full p-1 transition-colors disabled:opacity-30 disabled:hover:bg-transparent disabled:hover:text-gray-400 disabled:cursor-not-allowed"><svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg></button>
                                   </div>
                               ))}
                               {(!settings.branchSectors?.[sectorBranchFilter] || settings.branchSectors[sectorBranchFilter].length === 0) && <p className="text-gray-400 italic text-sm w-full text-center py-4">Nenhum setor vinculado a esta filial.</p>}
                           </>
                       ) : (
       ...
       {/* ... (Previous content for roles, events, etc) ... */}
       {/* --- CONTENT: ROLES --- */}
       {activeTab === 'roles' && (
           <div className="space-y-6 animate-fadeIn">
               {/* ... (Roles implementation preserved) ... */}
               ...
               <div className="bg-white rounded-xl shadow-lg border border-gray-100 p-6">
                   ...
                   <div className="flex flex-col md:flex-row gap-4 mb-8 bg-gray-50 p-4 rounded-xl border border-gray-200 items-start">
                       ...
                       <div className="w-full md:w-64">
                           ...
                           <select disabled={!hasPermission('settings:manage_roles_list')} value={newRoleMirrorSource} onChange={e => setNewRoleMirrorSource(e.target.value)} className="w-full border border-gray-300 rounded-lg p-2 text-sm bg-white disabled:bg-gray-100"><option value="">(Opcional)</option>{settings.roles.map(r => <option key={r.name} value={r.name}>{r.name}</option>)}</select>
                       </div>
       ...
       {/* --- CONTENT: EVENTS, ROTATIONS, TEMPLATES, INTEGRATIONS, SEASONAL, SYSTEM, SKILLS (Preserved) --- */}
       {/* (Keeping all other tabs content exactly as they were to maintain functionality) */}
       {/* ... */}
       {activeTab === 'events' && (
           <div className="space-y-6 animate-fadeIn">
               <div className="bg-white rounded-xl shadow-lg border border-gray-100 p-6">
                   ...
                   <div className="flex flex-col md:flex-row gap-4 mb-8 p-4 bg-gray-50 rounded-xl border border-gray-200 items-end">
                       ...
                       <div className="w-full md:w-48"><label className="text-xs font-bold text-gray-500 uppercase block mb-1">Comportamento</label><select disabled={!hasPermission('settings:edit_event_types')} value={newEventBehavior} onChange={e => setNewEventBehavior(e.target.value as any)} className="w-full border border-gray-300 rounded-lg p-2 text-sm bg-white disabled:bg-gray-100"><option value="neutral">Neutro</option><option value="debit">Débito</option><option value="credit_1x">Crédito (1x)</option><option value="credit_2x">Crédito (2x)</option></select></div>
       ...
       {activeTab === 'integrations' && (
           <div className="bg-white rounded-xl shadow-lg border border-gray-100 p-6 animate-fadeIn">
               ...
               <div className="grid grid-cols-1 md:grid-cols-3 gap-8"><div className="bg-gray-50 p-5 rounded-xl border border-gray-200 h-fit"><label className="text-xs font-bold text-gray-500 uppercase block mb-3">1. Filial Principal (Líder)</label><select className="w-full border border-gray-300 rounded-lg p-3 bg-white shadow-sm focus:ring-2 focus:ring-indigo-500 outline-none" value={selectedBranchForLinks} onChange={e => setSelectedBranchForLinks(e.target.value)}><option value="">Selecione...</option>{settings.branches.map(b => <option key={b} value={b}>{b}</option>)}</select></div>{selectedBranchForLinks && (<div className="md:col-span-2 bg-white p-6 rounded-xl border border-indigo-100 shadow-sm relative overflow-hidden"><div className="absolute top-0 left-0 w-1 h-full bg-indigo-500"></div><label className="text-xs font-bold text-gray-500 uppercase block mb-4">2. Marque as filiais visíveis</label><div className="grid grid-cols-1 sm:grid-cols-2 gap-3">{settings.branches.filter(b => b !== selectedBranchForLinks).map(b => { const isLinked = settings.branchLinks?.[selectedBranchForLinks]?.includes(b); const canEdit = hasPermission('settings:edit_integrations'); return (<label key={b} className={`flex items-center gap-3 p-3 rounded-lg border transition-all ${isLinked ? 'bg-indigo-50 border-indigo-200 ring-1 ring-indigo-200' : 'hover:bg-gray-50 border-gray-200'} ${!canEdit ? 'cursor-not-allowed opacity-60' : 'cursor-pointer'}`}><div className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${isLinked ? 'bg-indigo-600 border-indigo-600' : 'bg-white border-gray-300'}`}>{isLinked && <svg className="w-3 h-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>}</div><input type="checkbox" checked={!!isLinked} onChange={() => canEdit && toggleBranchLink(b)} disabled={!canEdit} className="hidden" /><span className={`text-sm ${isLinked ? 'font-bold text-indigo-900' : 'text-gray-700'}`}>{b}</span></label>); })}</div></div>)}</div>
           </div>
       )}
       {activeTab === 'seasonal' && (
           <div className="bg-white rounded-xl shadow-lg border border-gray-100 p-6 animate-fadeIn">
               <SectionHeader title="Eventos Sazonais" description="Períodos especiais que aparecem destacados no calendário." />
               <div className="grid grid-cols-1 md:grid-cols-12 gap-4 mb-8 p-5 bg-gray-50 rounded-xl border border-gray-200 items-end">
                   <div className="md:col-span-4"><label className="text-xs font-bold text-gray-500 uppercase block mb-1">Nome do Evento</label><input type="text" disabled={!hasPermission('settings:edit_seasonal')} value={newSeasonal.label} onChange={e => { setNewSeasonal({...newSeasonal, label: e.target.value}); validate('seasonalLabel', e.target.value); }} className="w-full border rounded-lg p-2 text-sm outline-none" /></div>
                   <div className="md:col-span-3"><label className="text-xs font-bold text-gray-500 uppercase block mb-1">Início</label><input type="date" disabled={!hasPermission('settings:edit_seasonal')} value={newSeasonal.startDate} onChange={e => setNewSeasonal({...newSeasonal, startDate: e.target.value})} className="w-full border rounded-lg p-2 text-sm" /></div>
                   <div className="md:col-span-3"><label className="text-xs font-bold text-gray-500 uppercase block mb-1">Fim</label><input type="date" disabled={!hasPermission('settings:edit_seasonal')} value={newSeasonal.endDate} onChange={e => setNewSeasonal({...newSeasonal, endDate: e.target.value})} className="w-full border rounded-lg p-2 text-sm" /></div>
                   <div className="md:col-span-1"><label className="text-xs font-bold text-gray-500 uppercase block mb-1">Cor</label><input type="color" disabled={!hasPermission('settings:edit_seasonal')} value={newSeasonal.color} onChange={e => setNewSeasonal({...newSeasonal, color: e.target.value})} className="w-full h-[38px] border rounded-lg cursor-pointer" /></div>
                   <div className="md:col-span-1"><button disabled={!hasPermission('settings:edit_seasonal')} onClick={addSeasonal} className="w-full bg-indigo-600 text-white h-[38px] rounded-lg font-bold hover:bg-indigo-700 shadow-sm flex items-center justify-center disabled:opacity-50">{Icons.Plus}</button></div>
               </div>
               <div className="space-y-3">{(settings.seasonalEvents || []).map(s => (<div key={s.id} className="flex justify-between items-center p-4 border rounded-xl hover:shadow-md transition-all bg-white group" style={{ borderColor: s.color, borderLeftWidth: '6px' }}><div><div className="font-bold text-gray-800 text-lg">{s.label}</div><div className="text-xs text-gray-500 font-medium flex gap-2 mt-1"><span className="bg-gray-100 px-2 py-0.5 rounded">📅 {new Date(s.startDate).toLocaleDateString()}</span><span>até</span><span className="bg-gray-100 px-2 py-0.5 rounded">📅 {new Date(s.endDate).toLocaleDateString()}</span></div></div><IconButton disabled={!hasPermission('settings:edit_seasonal')} onClick={() => removeSeasonal(s.id)} icon={Icons.Trash} colorClass="text-red-400 hover:bg-red-50" /></div>))}</div>
           </div>
       )}
       {activeTab === 'system' && (
           <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-fadeIn">
               <div className="bg-white rounded-xl shadow-lg border border-gray-100 p-6 flex flex-col h-full"><SectionHeader title="Configuração do Aviso" description="Exibe uma mensagem importante no topo de todas as páginas." /><div className="space-y-6 flex-1"><div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl border border-gray-200"><span className="font-bold text-gray-700">Ativar Banner</span><Switch disabled={!hasPermission('settings:manage_system_msg')} checked={sysMsg.active} onChange={c => setSysMsg({...sysMsg, active: c})} /></div><div><label className="text-xs font-bold text-gray-500 uppercase block mb-2">Nível de Urgência</label><div className="grid grid-cols-3 gap-3">{['info', 'warning', 'error'].map((lvl) => (<button key={lvl} disabled={!hasPermission('settings:manage_system_msg')} onClick={() => setSysMsg({...sysMsg, level: lvl as any})} className={`py-2 rounded-lg text-sm font-bold border transition-all ${sysMsg.level === lvl ? (lvl === 'error' ? 'bg-red-50 border-red-500 text-red-700' : lvl === 'warning' ? 'bg-amber-50 border-amber-500 text-amber-700' : 'bg-blue-50 border-blue-500 text-blue-700') : 'bg-white border-gray-200 text-gray-500 hover:bg-gray-50'} disabled:opacity-50 disabled:cursor-not-allowed`}>{lvl === 'error' ? 'Crítico' : lvl === 'warning' ? 'Alerta' : 'Informativo'}</button>))}</div></div><div><label className="text-xs font-bold text-gray-500 uppercase block mb-2">Mensagem</label><textarea rows={5} disabled={!hasPermission('settings:manage_system_msg')} value={sysMsg.message} onChange={e => setSysMsg({...sysMsg, message: e.target.value})} className="w-full border border-gray-300 rounded-xl p-3 focus:ring-2 focus:ring-indigo-500 outline-none text-sm leading-relaxed disabled:bg-gray-100" placeholder="Digite o comunicado aqui..." /></div><div className="mt-auto pt-4"><button disabled={!hasPermission('settings:manage_system_msg')} onClick={saveSysMsg} className="w-full bg-emerald-600 text-white font-bold py-3 px-6 rounded-xl hover:bg-emerald-700 shadow-lg transition-transform active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed">Salvar e Publicar</button></div></div></div>
               <div className="flex flex-col"><div className="bg-white rounded-xl shadow-lg border border-gray-100 p-6 flex-1"><SectionHeader title="Pré-visualização em Tempo Real" description="Como o usuário verá o aviso." /><div className="flex-1 flex items-center justify-center bg-gray-100 rounded-xl border-2 border-dashed border-gray-200 p-8 min-h-[300px]">{sysMsg.active ? (<div className={`w-full rounded-lg p-4 shadow-lg flex items-start ${sysMsg.level === 'error' ? 'bg-red-600 text-white' : sysMsg.level === 'warning' ? 'bg-amber-600 text-white' : 'bg-blue-600 text-white'}`}><div className="flex gap-3 w-full"><div className="mt-0.5 text-2xl">{sysMsg.level === 'error' ? '🚨' : sysMsg.level === 'warning' ? '⚠️' : 'ℹ️'}</div><div><h3 className="font-bold uppercase text-sm mb-1 tracking-wide">{sysMsg.level === 'error' ? 'MANUTENÇÃO / ERRO CRÍTICO' : sysMsg.level === 'warning' ? 'ATENÇÃO NECESSÁRIA' : 'COMUNICADO'}</h3><p className="text-sm whitespace-pre-wrap leading-relaxed opacity-95 font-medium">{sysMsg.message || "(Sua mensagem aparecerá aqui)"}</p></div></div></div>) : (<div className="text-center opacity-50"><div className="text-4xl mb-2">👻</div><p className="font-bold text-gray-500">O banner está desativado.</p><p className="text-xs text-gray-400">Ative para ver o preview.</p></div>)}</div></div></div>
           </div>
       )}
       {activeTab === 'skills' && (
           <div className="bg-white rounded-xl shadow-lg border border-gray-100 p-6 animate-fadeIn">
       ...
       {activeTab === 'backup' && hasPermission('settings:manage_backup') && (
           <div className="space-y-6 animate-fadeIn">
               <SectionHeader title="Backup e Restauração" description="Gerencie os dados do sistema. Cuidado: A restauração substitui todos os dados atuais." />
               
               <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                   {/* CARD DE EXPORTAÇÃO */}
                   <div className="bg-white p-8 rounded-xl shadow-lg border border-gray-100 flex flex-col items-center text-center hover:shadow-xl transition-all">
                       ...
                       <h3 className="text-xl font-bold text-gray-800 mb-2">Exportar Backup Completo</h3>
                       <p className="text-sm text-gray-500 mb-6 max-w-xs">
                           Gera um arquivo JSON contendo todos os dados do sistema: colaboradores, eventos, configurações e histórico.
                           <br/><span className="text-emerald-600 font-bold text-xs">Recomendado antes de grandes alterações.</span>
                       </p>
                       <button 
                           onClick={handleExportBackup}
                           disabled={isExporting}
                           className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3 px-6 rounded-lg shadow-md transition-transform active:scale-95 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                       >
                   ...
                   {/* CARD DE IMPORTAÇÃO */}
                   <div className="bg-white p-8 rounded-xl shadow-lg border border-red-100 flex flex-col items-center text-center hover:shadow-xl transition-all relative overflow-hidden">
                       <div className="absolute top-0 left-0 w-full h-1 bg-red-500"></div>
                       <div className="w-16 h-16 bg-red-100 rounded-full flex items-center justify-center mb-4 text-red-600">
                           <svg className="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>
                       </div>
                       <h3 className="text-xl font-bold text-gray-800 mb-2">Restaurar Dados</h3>
                       <p className="text-sm text-gray-500 mb-6 max-w-xs">
                           Importa um arquivo de backup para restaurar o estado do sistema.
                           <br/><span className="text-red-600 font-bold text-xs uppercase">Atenção: Isso substituirá TODOS os dados atuais!</span>
                       </p>
                       ...
                       <div className="w-full relative">
                           ...
                           />
                           <button 
                               className="w-full bg-red-600 hover:bg-red-700 text-white font-bold py-3 px-6 rounded-lg shadow-md transition-transform active:scale-95 flex items-center justify-center gap-2 disabled:opacity-50 z-10 relative pointer-events-none"
                           >
               ...
               <div className="bg-blue-50 border-l-4 border-blue-500 p-4 rounded-r-lg flex gap-3 items-start mt-6">
                   <svg className="w-6 h-6 text-blue-500 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                   <p className="text-sm text-blue-800">
                       <span className="font-bold">Nota de Segurança:</span> A funcionalidade de restauração é crítica. Certifique-se de que o arquivo JSON provém de uma exportação válida deste sistema. Todas as configurações locais, usuários e históricos serão revertidos para o estado do arquivo carregado.
                   </p>
               </div>
           </div>
       )}

       {/* MODALS (Permissions, Mirroring, Templates) - Preserved */}
       {/* 1. PERMISSIONS MODAL */}
       <Modal 
