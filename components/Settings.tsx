...
// --- COMPONENTE INTERNO: LISTAS SIMPLES ---
const ManageList = ({ 
  title, items, onAdd, onRemove, saving, removingId, placeholder 
}: {
  title: string; items: string[]; onAdd: (val: string) => void; onRemove: (val: string) => void; saving: 'idle' | 'saving' | 'success'; removingId: string | null; placeholder: string;
}) => {
            ...
            <div className="max-h-48 overflow-y-auto space-y-1 pr-1 scrollbar-thin">
              {filteredItems.map(item => (
                <div key={item} className="flex justify-between items-center p-2 hover:bg-gray-50 rounded group border-b border-gray-50 last:border-0">
                  ...
                  <button onClick={() => onRemove(item)} disabled={removingId === item} className="text-red-400 hover:text-red-600 text-xs font-bold px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity disabled:opacity-50">{removingId === item ? '...' : 'Excluir'}</button>
                </div>
              ))}
            </div>
          </div>
...
export const Settings: React.FC<SettingsProps> = ({ settings, setSettings, showToast }) => {
  ...
  const addRole = () => {
    ...
    // Default Permissions for new role: View Dashboard/Calendar only
      ...
      {activeSubTab === 'geral' && (
        <div className="animate-fadeIn space-y-8">
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <ManageList title="Filiais" items={settings.branches} onAdd={addBranch} onRemove={removeBranch} saving={savingState['branch'] || 'idle'} removingId={removingId} placeholder="Nova Filial..." />
              <ManageList title="Setores / Squads" items={settings.sectors || []} onAdd={addSector} onRemove={removeSector} saving={savingState['sector'] || 'idle'} removingId={removingId} placeholder="Novo Setor..." />
              <ManageList title="Perfis de Acesso (Sistema)" items={settings.accessProfiles || ['admin', 'colaborador', 'noc']} onAdd={addProfile} onRemove={removeProfile} saving={savingState['profile'] || 'idle'} removingId={removingId} placeholder="Novo Perfil (ex: supervisor)..." />
           </div>
           ...
           <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
             ...
             <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4 bg-gray-50 p-4 rounded-lg">
                ...
                <button onClick={addEvent} disabled={!newEventLabel.trim() || savingState['event'] === 'saving'} className="bg-indigo-600 text-white font-bold rounded-lg hover:bg-indigo-700">Adicionar</button>
             </div>
             <div className="space-y-2">
                {settings.eventTypes.map(e => (
                   <div key={e.id} className="flex justify-between items-center p-2 border-b border-gray-100 last:border-0">
                      <div><span className="font-bold">{e.label}</span> <span className="text-xs text-gray-500 ml-2">({e.behavior})</span></div>
                      <button onClick={() => removeEvent(e.id)} className="text-red-500 text-xs font-bold bg-red-50 px-2 py-1 rounded">Excluir</button>
                   </div>
                ))}
             </div>
           </div>
                   ...
                   <div className="space-y-1">
                      {settings.roles.map(r => (
                        <div 
                          ...
                          className={`flex justify-between items-center p-3 rounded-lg cursor-pointer transition-colors ${selectedRoleForACL === r.name ? 'bg-indigo-50 border border-indigo-200' : 'hover:bg-gray-50 border border-transparent'}`}
                        >
                          <span className={`font-medium ${selectedRoleForACL === r.name ? 'text-indigo-700' : 'text-gray-700'}`}>{r.name}</span>
                          <button 
                            onClick={(e) => { e.stopPropagation(); removeRole(r.name); }}
      ...
      {activeSubTab === 'jornada' && (
        <div className="animate-fadeIn">
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
               <h2 className="text-lg font-bold text-gray-800 mb-4">Criar Modelo de Jornada</h2>
               <div className="mb-4 flex gap-2">
                  <input type="text" className="flex-1 border border-gray-300 rounded-lg p-2 text-sm outline-none" placeholder="Nome (Ex: Escala 12x36)..." value={templateName} onChange={e => setTemplateName(e.target.value)} />
                  <button onClick={saveTemplate} className="bg-indigo-600 text-white px-4 py-2 rounded-lg font-bold hover:bg-indigo-700">Salvar Modelo</button>
               </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {(settings.scheduleTemplates || []).map(t => (
                     <div key={t.id} className="flex justify-between items-center p-3 border border-gray-200 rounded-lg bg-gray-50">
                        <span className="font-bold text-gray-700">{t.name}</span>
                        <button onClick={() => removeTemplate(t.id)} className="text-red-500 bg-red-50 px-3 py-1 rounded text-xs font-bold">Excluir</button>
                     </div>
                  ))}
               </div>
            </div>
