...
import { MultiSelect } from './ui/MultiSelect'; // Importar MultiSelect
...
export const Settings: React.FC<SettingsProps> = ({ settings, setSettings, showToast, hasPermission }) => {
  ...
  const [templateBranches, setTemplateBranches] = useState<string[]>([]); // Novo: Array de filiais
  ...
  const saveTemplate = () => {
    ...
    // Save to branches array
    const dataToSave = {
        name: templateName.trim(),
        branches: templateBranches, // Use array
        schedule: templateSchedule
    };

    if (editingTemplateId) {
        const updatedTemplates = (settings.scheduleTemplates || []).map(t => t.id === editingTemplateId ? { ...t, ...dataToSave } : t);
        ...
        const newT: ScheduleTemplate = { id: generateUUID(), ...dataToSave };
  ...
  const loadTemplateForEdit = (t: ScheduleTemplate) => { 
      ...
      // Handle data migration logic: check branches array first, then fallback to single branch string
      const loadedBranches = t.branches && t.branches.length > 0 
          ? t.branches 
          : (t.branch ? [t.branch] : []);
          
      setTemplateBranches(loadedBranches);
    ...
    <div className="space-y-6">
      ...
      {activeSubTab === 'geral' && showGeral && (
        <div className="animate-fadeIn space-y-8">
           ...
           {/* ... (Conteúdo de Eventos Sazonais - Sem Alterações) ... */}
           {/* ... (Conteúdo de Gerenciar Filiais e Setores - Sem Alterações) ... */}
           
           {/* ... (Conteúdo de Tipos de Evento - Sem Alterações) ... */}
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
                 ...
                 <div className={`mb-4 flex flex-col md:flex-row gap-2 p-3 rounded-lg ${editingTemplateId ? 'bg-blue-50 border border-blue-100' : ''}`}>
                    ...
                    <div className="md:w-64">
                        <MultiSelect 
                            label="Filiais (Opcional)"
                            options={settings.branches}
                            selected={templateBranches}
                            onChange={setTemplateBranches}
                            ...
                            <div className="flex flex-col overflow-hidden mr-2">
                                ...
                                {t.branches && t.branches.length > 0 ? (
                                    <div className="flex flex-wrap gap-1 mt-1">
                                        {t.branches.map(b => (
                                            <span key={b} className="text-[10px] bg-indigo-100 text-indigo-700 px-1.5 py-0.5 rounded font-semibold">{b}</span>
                                        ))}
                                    </div>
                                ) : (
                                    // Fallback para legado 'branch' (string) ou Global
                                    t.branch ? (
